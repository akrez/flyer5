...
class TypepartController extends Controller
{
    ...
    public static $category = Type::CATEGORY_PART;
    ...
    public function actionSuggest()
    {
        ...
        $databaseResults = Type::find()
                        ->where(['categoryId' => Type::CATEGORY_PART])
                        ->andFilterWhere([
                            'OR',
                            ['LIKE', 'name', $term],
                            ['LIKE', 'shortname', $term],
        ...
        foreach ($databaseResults as $databaseResult) {
            $results[] = ['id' => $databaseResult->id, 'text' => $databaseResult->name . ' (' . $databaseResult->parent->name . ')'];
        }
